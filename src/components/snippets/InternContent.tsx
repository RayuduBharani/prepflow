    ...
    return (
        <div className="w-full h-full p-2 sm:p-4 space-y-4">
            ...
            <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
                {/* Filters Sidebar */}
                <div className="w-full lg:w-[300px] flex-shrink-0 space-y-4 sm:space-y-6 bg-background/50 p-3 sm:p-4 rounded-lg border">
                    {/* Search Input */}
                    <div className="relative w-full">
                        ...
                        <Input className="pl-10 bg-background" placeholder="Search internships..." />
                    </div>
                    ...
                    <ScrollArea className="h-auto lg:h-[calc(100vh-12rem)] pb-4">
                        <div className="space-y-6 pr-4">
                                ...
                                <div className="space-y-3">
                                    {['First Year', 'Second Year', 'Third Year', 'Final Year'].map((year) => (
                                        <div className="flex items-center space-x-2" key={year}>
                                            <Checkbox id={year} />
                                            <label htmlFor={year} className="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                                                {year}
                                            </label>
                                        </div>
                                    ))}
                            ...
                            {/* Stipend Range */}
                            <div className="space-y-4">
                                <div className="font-semibold">Stipend Range</div>
                                <div className="space-y-3">
                                    {[
                                        'Unpaid',
                                        '$500 - $1000/month',
                        ...
                        {internshipsData.map((internship, index) => (
                            <div key={index} className="group relative bg-background rounded-lg border hover:shadow-md transition-all duration-300">
                                ...
                                <div className="relative p-3 sm:p-4">
                                    ...
                                    {/* Duration and Type with adjusted spacing */}
                                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                                        <Badge variant="secondary">{internship.duration}</Badge>
                                        <Badge variant="outline">{internship.experience || 'Full-time'}</Badge>
                                    </div>
