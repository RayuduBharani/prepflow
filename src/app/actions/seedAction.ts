...
export async function seedData(formData: FormData): Promise<void> {
  ...
  try {
    console.log(`[${new Date().toISOString()}] Reading platform data from JSON file...`);
    ...
    console.log(`[${new Date().toISOString()}] Parsing problems and collecting tags...`);
    problems.forEach((problem, index) => {
      ...
      if ((index + 1) % Math.ceil(problems.length / 10) === 0 || index === problems.length - 1) {
        const percentage = Math.floor(((index + 1) / problems.length) * 100);
        console.log(`[${new Date().toISOString()}] Processed ${index + 1}/${problems.length} problems (${percentage}%).`);
      }
    })

    console.log(`[${new Date().toISOString()}] Starting parallel creation of topics, companies, main topics, and topic slugs...`);
    await Promise.all([
      prisma.problemTopic.createMany({
        data: Array.from(topicsToCreate).map(name => ({ name })),
        skipDuplicates: true,
      }).then(() => console.log(`[${new Date().toISOString()}] Topics created successfully.`)),

      prisma.problemCompany.createMany({
        data: Array.from(companiesToCreate).map(name => ({ name })),
        skipDuplicates: true,
      }).then(() => console.log(`[${new Date().toISOString()}] Companies created successfully.`)),

      prisma.problemMainTopic.createMany({
        data: Array.from(mainTopicsToCreate).map(name => ({ name })),
        skipDuplicates: true,
      }).then(() => console.log(`[${new Date().toISOString()}] Main topics created successfully.`)),

      prisma.problemTopicSlug.createMany({
        data: Array.from(topicSlugsToCreate).map(slug => ({ slug })),
        skipDuplicates: true,
      }).then(() => console.log(`[${new Date().toISOString()}] Topic slugs created successfully.`)),
    ]);
    ...
    console.log(`[${new Date().toISOString()}] Fetching created problems for further processing...`);
    ...
    console.log(`[${new Date().toISOString()}] Creating similar problem relationships...`);
    ...
    problems.forEach((problem, index) => {
      ...
      if ((index + 1) % Math.ceil(problems.length / 10) === 0 || index === problems.length - 1) {
        const percentage = Math.floor(((index + 1) / problems.length) * 100);
        console.log(`[${new Date().toISOString()}] Processed similar questions for ${index + 1}/${problems.length} problems (${percentage}%).`);
      }
    })
    ...
    if (similarProblemsData.length > 0) {
      ...
      console.log(`[${new Date().toISOString()}] Similar problem relationships created successfully.`);
    } else {
      console.log(`[${new Date().toISOString()}] No similar problem relationships to create.`);
    }

    console.log(`[${new Date().toISOString()}] Creating problem relationships with tags, companies, main topics, and slugs...`);
    for (let i = 0; i < createdProblems.length; i++) {
      ...
      if ((i + 1) % Math.ceil(createdProblems.length / 10) === 0 || i === createdProblems.length - 1) {
        const percentage = Math.floor(((i + 1) / createdProblems.length) * 100);
        console.log(`[${new Date().toISOString()}] Processed relationships for ${i + 1}/${createdProblems.length} problems (${percentage}%).`);
      }
    }
    console.log(`[${new Date().toISOString()}] Problem relationships created successfully.`);

    console.log(`[${new Date().toISOString()}] Data seeding process completed successfully.`);
  } catch (error) {
    throw error
    console.error(`[${new Date().toISOString()}] Error during data seeding:`, error);
    throw error;
  }
}
  ...
  try {
    for (let i = 0; i < tablesToDrop.length; i++) {
      ...
      if (!table || typeof table !== 'string') {
        console.warn(`[${new Date().toISOString()}] Skipping invalid table name:`, table);
        continue;
      }
      ...
      console.log(`[${new Date().toISOString()}] Successfully dropped table: ${table}`);
    }

    console.log(`[${new Date().toISOString()}] All specified tables dropped successfully.`);
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Error while dropping tables:`, error);
    throw error;
  } finally {
